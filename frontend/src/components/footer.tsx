...
export default function Footer() {
	return (
		<footer className="relative z-10 w-full overflow-y-visible pt-16 pb-8 mt-auto">
			<style>{`
		.glass {
		...
		.glass:where(.dark, .dark *) {
		  ...
		  background: radial-gradient(circle, #ffffff1a 0%, #1e00001a 60%, #f03aa844 100%) !important;
		  border: 1px solid #ffffff0d !important;
		}
	  `}</style>
			<div className="pointer-events-none absolute top-0 left-1/2 z-0 h-full w-full -translate-x-1/2 select-none overflow-hidden">
				<div className="absolute right-1/4 -bottom-24 h-80 w-80 rounded-full bg-primary-600/20 blur-3xl" />
			</div>
			<div className="pointer-events-none absolute top-0 left-1/2 z-0 h-full w-full -translate-x-1/2 select-none overflow">
				<div className="absolute top-0 left-1/4 h-72 w-72 rounded-full bg-primary-600/20 blur-3xl" />
			</div>
			<div className="glass relative mx-auto flex container flex-col items-center gap-8 rounded-lg px-6 py-10 md:flex-row md:items-start md:justify-between md:gap-12 shadow-md">
				<div className="flex flex-col items-center md:items-start">
					<Logo />
					<p className="text-foreground mb-6 max-w-xs text-center text-sm md:text-left">
						Descubre y comparte guías de tus juegos favoritos. Nuestra app te
						ayuda a encontrar estrategias, consejos y trucos creados por la
						comunidad para mejorar tu experiencia de juego.
					</p>
			...
			<div className="text-foreground relative z-10 mt-10 text-center text-xs">
				<span>
					&copy; {new Date().getFullYear()} Guideon. Todos los derechos
					reservados.
				</span>
			</div>
		</footer>
